...
namespace PoconoDB {
    ...
    template<typename T>
    inline logger& operator << ( logger& wrapper,const T& nonstr)
    {
    ...
    template<>
    inline logger& operator << ( logger& wrapper,const std::string& str)
    {
        //std::cout<<str;
        wrapper.log(str);
        return wrapper;
   ...
   inline void log(logger& wrapper,std::string str )
    {
        wrapper.log(str);
