...
//     bool debug = true;
     ...
     const static size_t BLOCK_SIZE = 64;
     extern offsetType STARTING_OFFSET_OF_COLLECITON_INDEXS ;//2048
     extern offsetType ENDING_OFFSET_OF_COLLECITON_INDEXS ;
     extern offsetType STARTING_OFFSET_OF_DATA_BLOCK;//2048
     extern bool debug;

   inline std::string getFullCollectionName(std::string name) {
   ...
   inline bool openFileIfItDoesntExist(const std::string& name) {
        std::fstream f;
   ...
   inline void truncateTheFile(const std::string& name) {
        FILE * pFile;
        pFile = fopen(name.c_str(), "w");
        const char* str = "";
        assert(pFile);
   ...
   inline size_t getEndOfFileOffset(std::string filename)
    {
        FILE *fp;
   ...
   inline offsetType getEndOfFileOffsetAsMultipleOfBlock(std::string filename,size_t blockSize)
    {
        FILE *fp;
        assert(!filename.empty());
        fp=fopen(filename.c_str(),"r+b");
   ...
   inline offsetType getEndOfFileDataBlockOffsetAsMultipleOfBlock(std::string filename,size_t blockSize)
    {
        FILE *fp;
        assert(!filename.empty());
        fp=fopen(filename.c_str(),"r+b");
    ...
    //-----------------------------------------------------------------------------
    inline std::string getTimeNowAsString()
    {
   ...
   inline long convertStringToLong(std::string str)
    {
        long i = atol (str.c_str());
        return i;
    }
   inline std::string convertLongToString(long num)
    {
        return toStr(num);
    }
...
//    }
   ...
   inline std::string random_string(std::string firstPart, size_t length) {
        auto randchar = []() -> char
        {
            const char charset[] =
            "0123456789"
    ...
    inline std::size_t calc_hash_code(std::string obj) {
   ...
   inline string calc_request_id() {
        return random_number<string>(32);
    }

  inline  void sleepViaBoost(int seconds)
    {
   ...
   inline std::string getTimeNowAsString_Old() {
   ...
   inline void getProperMessageAboutSize(std::string data,std::string& properMessageAboutSize)
    {
