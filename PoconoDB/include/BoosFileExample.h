...
#ifndef PicoDB_BoosFileExample_h
#define PicoDB_BoosFileExample_h
...
//    asdsasdasdasd
    ...
    public:
        ...
        ~test_log()
        {
            ...
            if ( !file )
            {
                mylogger << "*****Warning - can't open output file: "
                << pth.string() << "\n";
            }
...
int cpp_main( int argc, char ** argv )
{
    ...
    if ( argc <= 1 )
        mylogger << "Usage: bjam [bjam-args] | process_jam_log [--echo] [--create-directories] [--v2] [locate-root]\n"
    ...
    if ( boost_root.empty() )
    {
        mylogger << "must be run from within the boost-root directory tree\n";
        return 1;
    }
    ...
    mylogger << "boost_root: " << boost_root.string() << '\n'
    ...
    while ( std::getline( *input, line ) )
    {
        ...
        if ( line.find( "boost-test(" ) == 0 )
        {
            ...
            if ( pos + 1 != line.size() )
            {
                ...
                //      mylogger << test_name << ", " << info.type << ", " << info.file_path << "\n";
            }
            else
            {
                mylogger << "*****Warning - missing test path: " << line << "\n"
